...
class Ping(object):
	...
	def do(self):
		...
		if receive_time:
			...
			globvar.logger.info("ICMP pkt loss")
			self.print_failed()
	...
	def send_one_ping(self, current_socket):
		...
		ICMP_TYPE = type
		#reserved = RESERVED + random.randint(0,3)
		#print 'reserved: ', reserved
		# Make a dummy header with a 0 checksum.
		header = struct.pack(
			"!BBHHH", ICMP_TYPE, 0, checksum, self.own_id, self.seq_number
		)
		...
		try:
			current_socket.setsockopt(socket.SOL_IP, socket.IP_TTL, ttl)
			current_socket.setsockopt(socket.IPPROTO_IP, socket.IP_TOS, ecn)
	...
	def receive_one_ping(self, current_socket):
		...
		while True: # Loop while waiting for packet or timeout
			...
			globvar.logger.info('receive one ICMP response from ' + str(address))
			
			reply_addr = address[0]
			if reply_addr == '10.25.25.25':
				globvar.logger.info("recv_pkt: " + str(globvar.recv_pkt))
				globvar.recv_pkt = globvar.recv_pkt + 1
			...
			globvar.logger.info('ICMP header:' + str(icmp_header))
...
def ping(hostname, args_ttl, args_type, args_ecn, timeout=1000, count=1, packet_size=55, *args, **kwargs):
	global ttl, type, ecn
